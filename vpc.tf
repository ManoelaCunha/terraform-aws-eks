...
locals {
  vpc_id_eks  = var.use_default_vpc ? data.aws_vpc.selected[0].id : aws_vpc.custom[0].id
  subnets_eks = var.use_default_vpc ? slice(data.aws_subnets.selected[0].ids, 0, 3) : aws_subnet.custom[*].id
}
